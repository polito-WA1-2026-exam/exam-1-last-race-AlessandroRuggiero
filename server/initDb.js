...
async function initDb() {
  ...
  for (const [line, stations] of Object.entries(lines)) {
    ...
    for (const station of stations) {
      if (!stationsIds.has(station)) {
        const id = await run("INSERT INTO stations (name) VALUES (?)", [
          station,
        ]);
    ...
    console.log(
      `Line ${line} initialized with ${stations.length} stations (${reusedStations} reused)`,
    );
    for (let i = 0; i < stations.length - 1; i++) {
      await run(
        "INSERT INTO connections (station1_id, station2_id, line_id) VALUES (?, ?, ?)",
        [
  ...
  const totalPotentialStations = Object.values(lines).reduce(
    (sum, stations) => sum + stations.length,
    0,
  );
